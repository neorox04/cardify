...
class SubscriptionTest extends TestCase
{
    ...
    {
        // plans.blade.php extends the dashboard layout (auth required)
    ...
    {
        $user = $this->verifiedUser();
        $this->actingAs($user)->get('/checkout/success')->assertStatus(200);
    }

    public function test_cancel_page_loads_for_authenticated_user(): void
    {
        $user = $this->verifiedUser();
        $this->actingAs($user)->get('/checkout/cancel')->assertStatus(200);
    }
    ...
    public function test_seats_page_loads_for_company_admin_without_subscription(): void
    {
        ...
        // no subscription — controller handles null gracefully
    ...
    public function test_enterprise_contact_sends_email_with_valid_data(): void
    {
        ...
        // Mail::raw() sends a raw message (not a Mailable class)
        // Mail::raw() sends a raw message; assert via the session flash instead
        // Mail::assertSentCount(1) only counts Mailable objects, not raw messages.
    }
    ...
    {
        // Endpoint must be reachable without auth (Stripe calls it server-to-server).
